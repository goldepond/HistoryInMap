...
- **정확한 지형** — Natural Earth 물리 지도(해안선·강·호수), 줌인 시 50m 자동 전환. **🏔 지형 토글**로 산맥·사막·고원 등 *큰 줄기 지형(벡터)* 을 유형별 색+이름으로 표시(어느 줌에서도 선명, 오프라인)
...
| 물리 지형·지형 구역(벡터) | [Natural Earth](https://www.naturalearthdata.com/) (via [martynafford](https://github.com/martynafford/natural-earth-geojson)) — 해안선·강·호수 + 산맥·사막·고원 등 지형 구역 | **Public Domain** |
