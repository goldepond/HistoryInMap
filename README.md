...
- **정확한 지형** — Natural Earth 물리 지도(해안선·강·호수), 줌인 시 50m 자동 전환. **🏔 지형 토글**로 실제 위성 지형(NASA Blue Marble) 위에서 볼 수도 있음
- **정사각(EPSG:4326) 투영** — 면적 왜곡이 적고(그린란드 거대화 없음) 지형 이미지와 정렬됨
...
| 물리 지형(벡터) | [Natural Earth](https://www.naturalearthdata.com/) (via [martynafford](https://github.com/martynafford/natural-earth-geojson)) | **Public Domain** |
| 위성 지형 이미지 | NASA Blue Marble (via three-globe 예제 텍스처) — `data/basemap/earth-terrain.jpg` | **Public Domain** |
